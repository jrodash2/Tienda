...
def crear_venta_pos(*, usuario, cliente=None, items=None, monto_pagado=Decimal('0.00'), metodo_pago='efectivo', referencia='', observaciones=''):
    items = items or []
    if not items:
        raise ValidationError('Agregue al menos un producto a la venta.')
    with transaction.atomic():
        venta = Venta.objects.create(cliente=cliente, usuario=usuario, origen='pos', observaciones=observaciones)
        total = Decimal('0.00')
        for item in items:
            ...
            DetalleVenta.objects.create(venta=venta, producto=producto, cantidad=cantidad, precio_unitario=precio, precio_costo_unitario=producto.precio_costo or Decimal('0.00'))
        ...
        if monto_pagado > 0:
            PagoVenta.objects.create(venta=venta, monto=monto_pagado, metodo_pago=metodo_pago, referencia=referencia, usuario=usuario)
